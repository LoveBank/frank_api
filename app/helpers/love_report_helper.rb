module LoveReportHelper
  ...
  def average_rating(user)
    @user = user
    @entries = Entry.where('linked_profile_id =? and received = true and private != true and created_at >= ?',
  ...
  def last_weeks_average_rating(user)
    @user = user
    @entries = Entry.where('linked_profile_id =? and received = true and private != true and created_at BETWEEN ? and ?',
